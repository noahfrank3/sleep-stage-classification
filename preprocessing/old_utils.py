...
def get_edf_dir_name(study_name):
    return 'sleep-' + study_name

def get_subject_filename(study_name):
    return 'S' + study_name[0].upper() + '-subjects.xls'
...
# Log status messages for a process
def log_status(status_deque, end_var=0, delay=0.5):
    ...
    while True:
        ...
        if current_time - last_log_time >= delay:
